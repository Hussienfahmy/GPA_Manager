...
class SubjectDaoTest {
    ...
    @Test
    fun test() = runTest {
        val subjectsWithGrade = subjectDao.subjectsWithGrades.first()
    ...
    fun getSubjectsWithGrade() = runTest {
        val testGrade = gradeDao.getGradeByName(Grade.Name.A).first()

        subjectDao.updateGradeName(templateSubject.id, testGrade.fullName)

        val (_, grade) = subjectDao.subjectsWithGrades.map {
            it.toList()
        }.first().first()
    ...
    fun getSubjects() = runTest {
        val loaded = subjectDao.subjectsWithGrades.map { it.toList().first() }.first().first
    ...
    fun upsert() = runTest {
        ...
        val loaded =
            subjectDao.subjectsWithGrades.map { subjectGradeMap ->
    ...
    fun updateGradeOrder() = runTest {
        val testGradeName = Grade.Name.APlus
    ...
    fun noSemesterWork_maxGrade() = runTest {
        ...
        assertEquals(Grade.Name.A, loaded.metadata.maxGradeNameCanAchieve)
    }
    ...
    fun semesterWorkAndMaxGrade() = runTest {
        ...
        var loaded = subjectDao.subjectsWithGrades.map { it.toList().first() }.first().first

        assertEquals(Grade.Name.B, loaded.metadata.maxGradeNameCanAchieve)

        subjectDao.updateMidterm(templateSubject.id, 4.0)
        loaded = subjectDao.subjectsWithGrades.map { it.toList().first() }.first().first

        assertEquals(Grade.Name.CPlus, loaded.metadata.maxGradeNameCanAchieve)
    }
}