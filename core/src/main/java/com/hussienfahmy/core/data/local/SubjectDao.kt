...
interface SubjectDao {
        ...
        SELECT*,
            CASE 
                WHEN midterm is null AND practical is null AND oral is null 
                    THEN (SELECT grade.full_name as max_grade FROM grade
                                 WHERE grade.active ORDER BY grade.percentage DESC LIMIT 1)
                
                ELSE (SELECT grade.full_name  FROM grade WHERE grade.active 
            ...
            END as max_grade
        FROM subject LEFT JOIN grade ON gradeName = grade.full_name ORDER BY creditHours DESC
        """
    )
    ...
    suspend fun updateGradeName(subjectId: Long, gradeName: Grade.Name?)
