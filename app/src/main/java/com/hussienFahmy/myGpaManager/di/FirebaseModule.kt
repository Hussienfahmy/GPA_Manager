...
val firebaseModule = module {
    // Firestore, Storage, and Auth are all migrated to the GitLive Firebase-Kotlin-SDK
    // (multiplatform, kotlinx.serialization-based / coroutine-first). Dropped the Firestore
    // persistent-cache-size customization (previously CACHE_SIZE_UNLIMITED) since GitLive's
    // settings API differs; defaults still provide offline caching, just not explicitly
    // unlimited-sized - flagged as a deliberate simplification to revisit if offline cache size
